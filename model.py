...
class HenonLayer(nn.Module): 
    def __init__(self, input_dim, hid_dim, epsilon=1, num_maps=4):
        super().__init__()
...
class HenonNet(nn.Module):
    def __init__(self, input_dim, hid_dim, num_layers=2, epsilon=1, num_maps=4):
        super().__init__()
        self.hlayers = [HenonLayer(input_dim, hid_dim, epsilon, num_maps) for _ in range(num_layers)]
