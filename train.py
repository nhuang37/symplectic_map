...
if __name__ == '__main__':
    ...
    #save path
    outdir = f'{args.save_path}_layer={args.num_layers}_hid={args.hid_dim}_lr={args.lr}_tie={args.tie_weights}_ep={args.num_epochs}'
    ...
    for epoch in range(args.num_epochs):  
        ...
        if (epoch+1) % args.eval_every == 0:
            ...
            for (xv, xv_out) in val_loader:
                ...
                xv_out = xv_out.to(device)
                x_pred, v_pred = model(xv[:,0:1], xv[:,1:2])
                loss_all += F.mse_loss(torch.concatenate([x_pred,v_pred],axis=-1), xv_out).item()
